...
const releases = [
  {
    ...
    date: 'Coming Soon',
    tag: 'latest',
    description: 'Initial release — namespaces, cgroups v2, OverlayFS, basic networking.',
    assets: [
      { os: 'Linux (x86_64)', file: 'veil-linux-amd64', available: false },
      ...
      <main className="min-h-screen pt-32 pb-20 relative z-0">
        <div className="w-full max-w-4xl mx-auto px-6">
          ...
          <section className="w-full py-20 border-t border-gray-100 mt-12">
            <h2 className="text-xs font-bold tracking-wide uppercase text-gray-400 mb-6">All Versions</h2>
            <div className="space-y-10">
              {releases.map((release) => (
                <div key={release.version}>
                  <div className="flex flex-wrap items-center gap-3">
                    <h3 className="text-2xl font-black tracking-tight text-[#2d2d2d]">{release.version}</h3>
                    <span className="text-xs font-bold tracking-wide uppercase text-gray-400">{release.date}</span>
                    ...
                    {release.assets.map((asset) => (
                      <div key={asset.file} className="border border-gray-100 px-4 py-3 flex justify-between items-center gap-4">
                        <div>
                          ...
                          <code className="font-mono text-xs text-gray-400">{asset.file}</code>
                        </div>
                        {asset.available ? (
                          <button
                            type="button"
                            ...
                            onClick={() => {
                              window.location.href = `/download?os=${toOsQuery(asset.os)}`
                            }}
                          >
                            Download
                          </button>
                        ) : (
                          <span className="text-gray-300 text-xs font-bold tracking-wide uppercase border border-gray-200 px-4 py-2">
                            Coming Soon
                          </span>
                        )}
                      </div>
